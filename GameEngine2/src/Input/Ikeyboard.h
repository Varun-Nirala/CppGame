...
public:
	...
	explicit Ikeyboard(DisplayWindow* pWindow)
		: m_pWindow(pWindow)
	{}
	...
	{
		for (size_t i = 0, size = m_keyEvents.size(); i < size; ++i)
		{
			if (m_keyEvents[i].keycode == keyCode && m_keyEvents[i].action.down)
			{
				return true;
			}
		}
	...
	{
		m_keyEvents.push_back(keybutton);
...
protected:
	...
	std::vector<KeyButton>		m_keyEvents;
};
